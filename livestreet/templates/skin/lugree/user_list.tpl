			...
			{foreach from=$aUsersList item=oUserList}
				...
				<tr>
					...
					<td>
						{if $oUserCurrent}
							{if $oUserNote}
								<button type="button" class="button button-action button-action-note js-infobox" title="{$oUserNote->getText()|escape:'html'}"><i class="icon-synio-comments"></i></button>
							{/if}
					...
					<td class="cell-rating {if $oUserList->getRating() < 0}negative{/if}">{$oUserList->getRating()}</td>
				</tr>
			{/foreach}
		{else}
			<tr>
