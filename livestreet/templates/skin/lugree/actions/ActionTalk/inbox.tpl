...
{include file='header.tpl' noShowSystemMessage=false usermenu='menu.talk.tpl'}
...
{if $aTalks}
	...
	<form action="{router page='talk'}" method="post" id="form_talks_list">
		...
		<table class="table table-talk messages-list">
			<thead>
				<tr>
					...
					<tr>
						...
						<td class="cell-title">
							...
							{if $oTalk->getCountComment()}
								<span class="comments-count"><span>{$oTalk->getCountComment()}</span>{if $oTalkUserAuthor->getCommentCountNew()} <span class="new">+{$oTalkUserAuthor->getCommentCountNew()}</span>{/if} </span>
							{/if}
