jQuery(document).ready(function($){
	...
	// Кнопка "UP"
	ls.toolbar.up.init = function() {
        $(window).scroll(function(){
            if ($(window).scrollTop() > $(window).height() / 2) {
                $('#toolbar_scrollup').slideDown(200);
            } else {
                $('#toolbar_scrollup').slideUp(200);
            }
        });
    };
	ls.toolbar.up.init();
	...
	ls.hook.add('ls_blog_toggle_join_after',function(idBlog,result){
		if (!this.data('onlyText')) {
			this.html('<i class="icon-pencil"></i><span> '+(result.bState ? ls.lang.get('blog_leave') : ls.lang.get('blog_join'))+'</span>');
			if (result.bState) {
...
function toolbarPos() {
	...
	if ($('#toolbar section').length) {
		if ($(document).width() <= 1100) {
			$('#toolbar').css({'position': 'absolute', 'right': 0, 'top' : $(document).scrollTop() + 130, 'display': 'block'});
		} else {
			$('#toolbar').css({'position': 'fixed', 'right': 0, 'top': 130, 'display': 'block'});
		}
	}
};