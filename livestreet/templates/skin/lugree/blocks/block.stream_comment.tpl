<ul class="latest-list">
	{foreach from=$aComments item=oComment name="cmt"}
		...
		<li class="js-title-comment" title="{$oComment->getText()|strip_tags|trim|truncate:100:'...'|escape:'html'}">
			<p>
                <a href="{$oUser->getUserWebPath()}" class="author-avatar" title="{$oUser->getLogin()}"><img src="{$oUser->getProfileAvatarPath(48)}" alt="{$oUser->getLogin()}"></a>
			...
			<span class="block-item-comments">{$oTopic->getCountComment()}</span>
		</li>
	{/foreach}
</ul>
...
<footer>
	<a href="{router page='comments'}">{$aLang.block_stream_comments_all}</a> <a href="{router page='rss'}allcomments/"><i class="icon-rss"></i></a>
</footer>