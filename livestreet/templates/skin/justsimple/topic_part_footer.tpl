      		...
      		{if $bTopicList}
      			<li class="comments-link">
      				{if $oTopic->getCountComment()>0}
      					<a href="{$oTopic->getUrl()}#comments" title="{$aLang.topic_comment_read}">{$aLang.topic_comm}: {$oTopic->getCountComment()}</a><span>{if $oTopic->getCountCommentNew()} +{$oTopic->getCountCommentNew()}{/if}</span>
      				{else}
      					<a href="{$oTopic->getUrl()}#comments" title="{$aLang.topic_comment_add}">{$aLang.topic_comm}: {$oTopic->getCountComment()}</a>
      				{/if}
      			</li>
      		{/if}
			<li class="topic-info-share" data-topic-id="{$oTopic->getId()}" onclick="jQuery('#topic_share_{$oTopic->getId()}').slideToggle(); return false;"><i>{$aLang.share_topic}</i></li>
        	<li class="favor" onclick="return ls.favourite.toggle({$oTopic->getId()},$('#fav_topic_{$oTopic->getId()}'),'topic');">
